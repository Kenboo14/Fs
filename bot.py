    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNELS:
            try:
                ...
                self.LOGGER(__name__).info(
                    f"FORCE_SUB_CHANNELS detected!\n┌ Title: {info.title}\n└ Chat ID: {info.id}\n——"
                )
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning(
                    "Bot tidak dapat Mengambil link invite dari FORCE_SUB_CHANNELS!"
                )
                self.LOGGER(__name__).warning(
                    f"Pastikan @{self.username} adalah admin di Channel Tersebut, Chat ID F-Subs Channel Saat Ini: {FORCE_SUB_CHANNELS}"
                )
                self.LOGGER(__name__).info(
                    "Bot Berhenti."
                )
        ...
        if FORCE_SUB_GROUPS:
            try:
                info = await self.get_chat(FORCE_SUB_GROUPS)
                link = info.invite_link
                if not link:
                    await self.export_chat_invite_link(FORCE_SUB_GROUPS)
                    link = info.invite_link
                self.invitelink4 = link
                self.LOGGER(__name__).info(
                    f"FORCE_SUB_GROUPS detected!\n┌ Title: {info.title}\n└ Chat ID: {info.id}\n——"
                )
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning(
                    "Bot tidak dapat Mengambil link invite dari FORCE_SUB_GROUPS!"
                )
                self.LOGGER(__name__).warning(
                    f"Pastikan @{self.username} adalah admin di Group Tersebut, Chat ID F-Subs Group Saat Ini: {FORCE_SUB_GROUPS}"
                )
                self.LOGGER(__name__).info(
                    "Bot Berhenti."
                )
